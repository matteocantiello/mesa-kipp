...
def matrix(mat,numz,values,q,min_mat,max_mat):
    k = 0
...
# Create Nuclear Energy Generation Matrix
    eps, min_eps, max_eps = matrix(eps,numz,epsnuc,q,min_eps,max_eps)
    neu, min_neu, max_neu = matrix(neu,numz,epsneu,q,min_neu,max_neu)

# Create Other Matrix 
    con, min_con, max_con = matrix(con,numz,conv_vel,q,min_con,max_con)
    bfi, min_bfi, max_bfi = matrix(bfi,numz,b_field,q,min_bfi,max_bfi)
#    ome, min_ome, max_ome = matrix(ome,numz,omega,q,min_ome,max_ome)
#    mst, min_mst, max_mst = matrix(mst,numz,DST,q,min_mst,max_mst)
#    msi, min_msi, max_msi = matrix(msi,numz,DSI,q,min_mst,max_msi)
#    mes, min_mes, max_mes = matrix(mes,numz,DES,q,min_mes,max_mes)   
...
if len(TITLE) > 0:      # USER Defined Title
    ...
    mm=prof.header_attr.get('initial_mass')
    zz=prof.header_attr.get('initial_z')
    nn=prof.header_attr.get('version_number')
